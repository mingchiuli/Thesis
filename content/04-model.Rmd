...
实际汇率 & 某国家或地区某年的实际有效汇率（Reer） & 布鲁盖尔研究所（Bruegel AISBL）\\
\hline
\end{tabular}
\label{tab:data}
\end{table}
...
jan_imp <- base_imp |> 
  filter(State == 'Japan') |> 
  group_by(Year) |> 
  mutate(sum_imp = sum(ImpPerc))

er_imp <- base_imp |> 
  filter(State %in% EURO) |> 
  group_by(Year) |> 
  mutate(sum_imp = sum(ImpPerc, na.rm = TRUE))
...
\begin{landscape}
```{r 19942016map, echo=FALSE, fig.cap='1994年到2016年台湾地区进出口比重累积变化', warning=FALSE, fig.align='center',fig.width=16.5, fig.height=11.7}

c(1994, 2016) |> 
  purrr::map(~ tw_data |> 
               filter(Year == .) |> 
               mutate(百分比变动 = (ExpPerc + ImpPerc) * 100, .keep = "none")) |> 
  reduce(~ .y - .x) |> 
...
```{r fta, echo=FALSE, fig.cap='台湾对自贸协定国家进出口总额滞后差分', warning=FALSE,fig.width=4.41, fig.height=8.98}

gene_fta <- function(state) {
  tw_data |> 
  filter(State == state, Year >= tw_data |> 
           filter((FTA == 1 & State == state) | (ECFA == 1 & State == state)) |> 
           select(Year) |> 
           slice(1) |> 
           as.numeric() - 1) |> 
  pull(Export + Import) |> 
