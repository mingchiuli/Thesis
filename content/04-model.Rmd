...
在南向政策的实施中，因其涉及涉及两个政策干涉时间点，DID产生多干预时点的情形，模型\@ref(eq:twdid)转化为：
...
在上述研究的基础上，本文希望进一步探究对于南向政策和新南向政策的核心实施国家的政策干预效应。对于新南向政策的核心实施国家，本文参考2016年9月19日台湾当局“外交部长”李大维参加相关会议论坛提出的国家进行划分。对于南向政策时期的研究，本文将GSP1.1所涉及国家设置为“重点国家”。模型设置如下：

\begin{equation}
Perc_{it} = \beta \cdot Treat_{it} \cdot G_{i} + \alpha \cdot G_{i} + \gamma \cdot Z + \lambda_{i} + \mu_{t} + \epsilon_{it}(\#eq:twdidheter)
\end{equation}

模型\@ref(eq:twdidheter)使用异质性分析的思路，将政策干预个体分为核心国家与非核心国家，如果个体$i$属于设定的核心国家，则$G_{i}$取1，否则取0。通过观察回归系数$\alpha$的显著性可得出政策干预核心国家的实施效果。若$\alpha$在95%的置信区间上显著异于0，则说明政策干预对核心国家是有效的。

## 变量和数据来源

我们从台湾“财政部”、“投资审查委员会”获得了台湾对世界各国家或地区的进口、出口及对外直接投资（FDI）数据，从世界银行数据库和布鲁盖尔研究所获得了控制变量的相关数据，相关信息如表\@ref(tab:data)所示。
...
台湾对外贸易协定可分为经济合作协定（ECA）、自由贸易协定（FTA）以及两岸经济合作协议（ECFA），在台湾当局“经济部国际贸易局”网站^[网址：https://fta.trade.gov.tw/。]中三者并列地位同等，故本文将三者统称为自贸协定。截止到2019年，台湾与新加坡、巴拿马、危地马拉、洪都拉斯、尼加拉瓜、巴拉圭、新西兰、斯威士兰以及中国大陆建立了自贸协定。
