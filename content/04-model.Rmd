...
双重差分法（Difference in Differences，DID）是政策评估的非实验方法中最为常用的一种计量方法。对于某种政策的实施，存在受政策影响的个体和不受政策影响的个体，分别称为实验组和对照组。双重差分法的应用基于这种事实：如果政策影响前，实验组和对照组具有相同的变化趋势，而政策发生后两者产生了不同的变化趋势，则说明政策是有效的。从类别上，对于双重差分的应用，大致可以分为传统DID（Standard DID）和多期DID（Time-varying DID），前者的应用场景在于，所有实验组个体受政策冲击的时点是一致的，而后者的应用场景在于，不同的实验组个体受到了不同时点的政策冲击影响。
...
图\@ref(fig:DID)中灰色实线段表示的是假设政策冲击未发生时，实验组“应有”的变化趋势。若政策冲击发生后实验组变化趋势由黑色虚线段所表示，则反映了实验组受政策的影响是区别于对照组的。上图中的垂直方向实线即为DID识别的因果效应。在回归分析中，如果因果效应的回归系数显著，就可以说明政策干预对于实验组产生的影响是区别于没有政策干预的对照组的，因为DID方法已经剔除了实验组和对照组共同的变化趋势。
...
于是$\beta$系数可以解释为政策干预的实际效果。在双向固定效应模型中，若在个体层面和年度层面对回归进行控制，则可以得到比仅控制组别和政策干预前后更为精细的回归系数。而模型\@ref(eq:mean)中的交乘项已因双向固定效应的加入产生完全共线性而消除。

在模型\@ref(eq:twfe)中，如果个体i在t期受到干预，则$D_{it}$取1，否则取0。双向固定效应对于传统形式的DID模型而言，控制粒度更细，在控制了时间和个体的情况下去寻找政策产生的效应，而非仅仅控制政策前后和实验对照分组，具有更好的准确性。另外，对于非传统形式的DID，即多期DID，也适用于这种情况。
...
在模型\@ref(eq:twdid)中，$Perc_{it}$表示台湾对$i$地区在$t$年的进口、出口及投资占当年总量的比例，$Southbound_{i}$代表$i$国或地区是否为受政策影响的个体，$Post_{it}$代表$i$国或地区在$t$年是否处于政策干预后，$Southbound_{i} \cdot Post_{t}$代表当年该国是否受到了新南向政策的干预，受到干预取1，否则取0。$\lambda_{i}$和$\mu_{t}$分别为个体固定效应和时间固定效应，$Z$为控制变量，$\epsilon_{it}$为误差项。$\beta$就是我们关心的系数，如果此系数显著，则在平行趋势成立的条件下，南向政策或新南向政策对相关贸易比例变量产生了影响。
...
进一步的，在运用DID模型探讨政策效果的基础上，我们还希望被解释变量在政策干预前后年份分布的动态变化，同时双重差分法运用有效的前提是政策干预组和对照组需要满足平行趋势检验，即处理组和对照组的变化趋势在政策干预前没有显著性差异。参考Beck, et al.  (2010)研究中的平行趋势检验设定[@beck2010big]，本文对平行趋势检验回归模型设置如下：
...
模型\@ref(eq:twevent)基于事件研究法，将研究范围分为政策干预前共$\tau_{0}$期和政策干预后共$\tau_{1}$期，$\tau=-1$代表政策干预的前一期，作为研究基准期。$D_{it}$是个体在某年是否受政策干预的虚拟变量，以政策干预的前一年为基期，如果观测值所处年份距离政策干预${k}$年，则$\alpha_{k}$对应的$D_{i,t}$取1，其他所有$D_{i,t}$取0。如果回归结果在95%的置信区间上满足回归系数从$\alpha_{-\tau_{0}}$到$\alpha_{-2}$都不显著异于0，则被解释变量的在政策干预前年份的分布效应的与基期没有显著性差异。在验证平行趋势检验的方法中，本文将回归系数的95%置信区间使用误差棒的形式在图形中绘出，若在基期前所有误差棒都穿过0，则平行趋势假设得到满足。同时，若在基期后某年误差棒不穿过0，则认为在该期政策干预具有显著的滞后效应。
...
根据黄奎博等(2014)的研究，2003年后台湾当局对南向政策便甚少提及，故本文以此作为南向政策终点的划分依据。根据该研究，南向政策时期，台湾当局主要做了两次调整，第一次在1994-1996年划定了首批国家，分别为泰国、马来西亚、印度尼西亚、菲律宾、新加坡、越南和文莱。Bing, et al.  (2017)的研究将上述时期称为“Go South Policy1.1”，即GSP1.1[@bing2017taiwan]；第二次是1997-1999年，在7国的基础上增加至12国，在GSP1.1的基础上加入老挝、缅甸、柬埔寨、澳大利亚以及新西兰，对应为GSP1.2。南向政策最终在GSP1.2所涵盖范围的基础上，延长到2002年底为止[@黄奎博]。由于数据完整性的原因，我们在受政策干预12国的基础上加入了一部分与台湾贸易较为密切的国家或地区作为研究样本。分别为中国大陆、印度、巴基斯坦、孟加拉国、不丹、斯里兰卡、尼泊尔、马尔代夫、巴布达新几内亚、帕劳、基里巴斯、瑙鲁、新喀里多尼亚、瓦努阿图、萨摩亚、日本、韩国、中国香港、美国、中国澳门、英国、法国、德国、西班牙、意大利、加拿大和荷兰。在南向政策期间，台湾对这些国家或地区的年度总进出口额均占台湾年度对外总进出口额的80%以上，具有较强的代表性。在之后的研究中，将此研究阶段的南向政策统称为“一次南向”。

台商向中国大陆投资的传统途径，或经过第三国向中国大陆投资，或须通过台湾当局的审查方可进行。2010年，马英九政府与大陆签署了《海峡两岸经济合作框架协议》，倡导放宽投资限制。马英九政府执政时期的经济政策同蔡英文政府力推南向经济形成了鲜明的反差，故本文以马英九执政作起点为新南向研究时间起点，将新南向的研究时间划定为2008-2019年，以蔡英文当局正式提出新南向政策的2016年作为政策干预起点。根据台湾当局“投资审查委员会”资料，新南向受政策干预国家有18个，被称为“新南向18国”，分别为：泰国、马来西亚、印度尼西亚、菲律宾、新加坡、越南、文莱、老挝、缅甸、柬埔寨、澳大利亚、新西兰、印度、巴基斯坦、孟加拉国、不丹、斯里兰卡和尼泊尔。在新南向政策的研究中，本文使用了全部的数据样本，共包含201个国家或地区的数据。在之后的研究中，将此研究阶段的南向政策统称为“二次南向”。
...
jan_exp <- base_exp |> 
  filter(State == 'Japan') |> 
  group_by(Year) |> 
  mutate(sum_exp = sum(进口占比))

er_exp <- base_exp |> 
  filter(State %in% EURO) |> 
  group_by(Year) |> 
  mutate(sum_exp = sum(进口占比, na.rm = TRUE))
...
jan_imp <- base_imp |> 
  filter(State == 'Japan') |> 
  group_by(Year) |> 
  mutate(sum_imp = sum(出口占比))

er_imp <- base_imp |> 
  filter(State %in% EURO) |> 
  group_by(Year) |> 
  mutate(sum_imp = sum(出口占比, na.rm = TRUE))
...
图\@ref(fig:1981fdi)展示了台湾地区对主要贸易地区的直接投资比重随时间的变化趋势。由图可以得知，台湾对其贸易伙伴的投资具有较强的波动性。1980年代，台湾对外主要投资在东盟与北美自贸区之间交织波动，例如1982年，台湾对东盟投资占比超过75%，对北美自贸区占比低于25%，而到了1985年台湾对北美自贸区投资占比接近80%，而此时对东盟投资占比则低于12.5%，体现出台湾地区投资极大的不确定性。从南向政策实施开始到1997年东南亚金融危机前，台湾对东盟投资比重总体上呈上升趋势，而东南亚金融危机发生后则急剧下降，可见东南亚金融危机对南向政策实施的效果影响较大。新南向政策实施后，台湾对东盟投资占比也呈现出强烈的波动性。台湾对大陆投资占比随着陈水扁政府上台就呈现上升趋势，到2010年已超过75%，而之后开始呈现出下降趋势。伴随着对中国大陆投资比重的下降，台湾对东盟以及加勒比海英国属地的间接投资也震荡上升，呈现出一定的周期性。总体来看，从2010年以来，部分原属中国大陆的投资比重受到东盟、间接投资（加勒比海英国属地）以及欧盟的蚕食，到2019年台湾对大陆的投资比重大约37.5%，台湾对东盟的投资比重则达到了25%左右。台湾对北美自贸区的投资比重从1985年到1994年急剧下降，此后始终在5%左右的水平徘徊。对于中国香港而言，投资比重从1981年的超过25%开始就开始降低，最终维持在5%左右波动。总之，对于台湾对外直接投资比重而言，变化趋势不稳定是其主要特征，而加勒比海英国属地则在投资转移的过程中扮演了一个重要的角色。

### 台湾对外贸易区位因素分析

图\@ref(fig:19942016map)^[数据来源：台湾财政部，图中贸易数据未包含中国香港和中国澳门。]展示了从1994年到2016年台湾对外贸易进出口总比例累积变化地图。从1994年到2016年，从一次南向的起点到二次南向的起点，台湾对外出口分布已经产生了很大的区位变化。其中，中国大陆（+44.0%）、韩国（+5.7%）、越南（+3.6%）、新加坡（+2.7%）、菲律宾（+2.2%）、印度（+1.1%）、沙特阿拉伯（+1.0%）、科威特（+1.0%）、卡塔尔（+0.9%）、阿联酋（+0.6%）位居台湾进出口总比例累积增长的前十，而美国（-23.4%）、日本（-15.0%）、中国香港（-10.0%）、德国（-3.2%）、英国（-2.0%）、加拿大（-1.8%）、南非（-1.3%）、印尼（-1.1%）、意大利（-0.9%）、法国（-0.8%）位居台湾进出口总比例累积下降的前十。结合图形可以看出，比重波动较为明显的地区有中国大陆、新加坡、美国、日本、韩国、越南、德国、菲律宾、印度等国家或地区。从台湾进出口总比重的变化来看，台湾进出口重心从欧洲和北美洲逐渐转移到了东亚和东南亚。从细节来看，在东亚范围内，台湾对日本的部分进出口份额一定程度上受到中国大陆以及韩国所威胁，2016年台湾对大陆的进出口比重比1993年的比重高出40%以上，美国则下降超过20%。从地理位置上解释上述变化，东亚及东南亚国家具有较密集的人口分布和较大的消费潜力，且距离台湾地区距离较近，具有运输成本上的优势。另外，在这个时间段东南亚及东亚诸国也有着较快的经济增长速度，对于进出口贸易是利好因素。
...
台湾地区对外自贸协定可分为经济合作协定（ECA）、自由贸易协定（FTA）以及两岸经济合作协议（ECFA），在台湾当局“经济部国际贸易局”网站^[网址：https://fta.trade.gov.tw/。]中三者并列地位同等，本文将三者统称为自贸协定。截止到2019年，台湾与新加坡、巴拿马、危地马拉、洪都拉斯、尼加拉瓜、巴拉圭、新西兰、斯威士兰以及中国大陆建立了自贸协定。
...
图\@ref(fig:fta)为台湾对与其达成自贸协定的9个国家或地区滞后差分随年份的变化示意图，对于某个国家来说，台湾地区与其达成自贸协定为起点年份。其中，某年度对应的数额代表该年度进出口总额与上一年度进出口总额的差值。从上方的图中可以看到，在与台湾达成自贸协定后，巴拿马和危地马拉在多数年份保持了对台贸易总额的增长，而尼加拉瓜和洪都拉斯贸易总额增长较不明显。对于新西兰与新加坡而言，在与台湾达成自贸协定后，贸易总额都经历了先增后减再增的过程，呈现出震荡波动趋势。斯威士兰和巴拉圭由于协定实施年份较短，均是在与上年度进出口总额接近的水平上徘徊。

从下方的图出发，则可以看出自从2010年ECFA协议实施以来，台湾和中国大陆之间的贸易总额基本处于持续增长的状态，且增长的幅度远大于与台湾达成自贸协定的国家和地区。尽管这种增长未必可以完全归因于ECFA协定的签订，但仍可从中看出中国大陆与其他地区相比对台湾产生的巨大影响力。
