...
模型\@ref(eq:twevent)基于事件研究法，将研究范围分为政策干预前共$\tau_{0}$期和政策干预后共$\tau_{1}$期，$\tau=-1$代表政策干预的前一期，作为研究基准期。$D_{it}$是个体在某年是否受政策干预的虚拟变量，以政策干预的前一年为基期，如果观测值所处年份距离政策干预${k}$年，则$\alpha_{k}$对应的$D_{i,t}$取1，其他所有$D_{i,t}$取0。如果回归结果在95%的置信区间上满足回归系数从$\alpha_{-\tau_{0}}$到$\alpha_{-2}$都不显著异于0，则被解释变量的在政策干预前年份的效应的与基期没有显著性差异。在验证平行趋势检验的方法中，本文将回归系数的95%置信区间使用误差棒的形式在图形中绘出，若在基期前所有误差棒都穿过0，则平行趋势假设得到满足。同时，若在基期前某年开始误差棒均不穿过0，则认为政策干预具有显著的预期效应；若在基期后某年误差棒不穿过0，则认为在该期政策干预具有显著的滞后效应。
