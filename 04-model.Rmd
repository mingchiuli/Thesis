...
```{r 2000fdi, fig.cap='2008年以来台湾投资占比分布', echo=FALSE, warning=FALSE, fig.align = 'center'}
...
ggplot() + 
  ...
  geom_line(aes(Year, sum_fdi, col = '美国'), data = us_fdi) +
  geom_line(aes(Year, sum_fdi, col = '新南向国家'), data = nsbp_fdi) +
...
在新南向政策执行4年之后，如图\@ref(fig:NSBPPerc)所示，台湾对新加坡以及越南累计投资占比较大，分别为29.1%与26.2%，对澳大利亚以及泰国的投资占比位列第二梯队，分别为10.6%与9.7%，对斯里兰卡、巴基斯坦、不丹、尼泊尔等国的投资则始终没有进行过直接投资的行为，占比为0。这体现出新南向政策实施下台湾投资分布的区域不均衡性。
